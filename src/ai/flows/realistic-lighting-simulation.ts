...
const GenerateRealisticPreviewInputSchema = z.object({
  ballDesignDataUri: z
    .string()
    .describe(
      "A data URI of the golf ball design (logo or text). Expected format: 'data:<mimetype>;base64,<encoded_data>' for images, or 'data:text/plain;charset=utf-8,<encoded_text>' for text."
    ),
  lightingCondition: z
    .string()
...
const generateRealisticPreviewFlow = ai.defineFlow(
  ...
  async input => {
    let prompt;
    let promptInput: { text: string; media?: { url: string } } = { text: '' };

    if (input.ballDesignDataUri.startsWith('data:image')) {
        // Handle image input
        promptText = `Generate a photorealistic image of a custom golf ball. The ball has the provided logo printed on it. The lighting should be ${input.lightingCondition}, and the viewing angle should be ${input.angle}.`;
        promptInput = {
            text: promptText,
        ...
        promptText = `Generate a photorealistic image of a custom golf ball. The ball has the text "${textContent}" printed on it in a simple, clear font. The lighting should be ${input.lightingCondition}, and the viewing angle should be ${input.angle}.`;
        promptInput = {
            text: promptText
        };
    }
